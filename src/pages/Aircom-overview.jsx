      ...
      <div className="flex flex-col items-center bg-white p-6 rounded-3xl shadow-lg border border-gray-100 w-full">
        <div className="relative w-32 h-32">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] text-gray-400 uppercase font-bold">{unit}</span>
          </div>
        </div>
  ...
  return (
    ...
    {/*-----------------------------------------------------------------------------------------------------------------*/}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <Gauge value={currentData_ac101?.presAC1} max={10} label={`Pressure AC101`} unit="Bar" color="text-blue-500" />
        <Gauge value={currentData_ac101?.tempAC1} max={120} label={`Temp AC101`} unit="°C" color="text-orange-500" />
        <Gauge value={currentData_ac101?.currentAC1} max={100} label={`Current AC101`} unit="Amp" color="text-emerald-500" />
        <Gauge value={currentData_ac101?.freqAC1} max={100} label={`Freq AC101`} unit="Hz" color="text-purple-500" />
      </div>
      ...
      <div className="flex gap-2 mb-8 bg-gray-200 p-1 rounded-2xl w-full justify-between h-10">
        ...
        {/* <div className={`px-4 py-1 rounded-full text-[10px] font-black flex items-center uppercase shadow-sm ${connected_ac102 ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
          {connected_ac102 ? "● System Live" : "○ Disconnected"}
        </div> */}
        {/* {["AC101", "AC102", "AC103","STB101"].map((l) => (
          <button
            key={l}
